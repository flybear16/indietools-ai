...
import { getToolsWithStats, getAllCategories } from '@/lib/db/queries';
...
import { PRICING_MODELS, PHASES } from '@/types';
...
export default async function ToolsPage({ searchParams }: Props) {
  ...
  const toolsWithStats = await getToolsWithStats();
  ...
  const selectedPricing = pricingParam
    ? Array.isArray(pricingParam) ? pricingParam as string[] : [pricingParam as string]
    : [];
  ...
  // Get selected phase filter from URL
  const phaseParam = resolvedSearchParams.phase;
  const selectedPhase = typeof phaseParam === 'string' ? phaseParam : null;
  ...
  let filteredTools = toolsWithStats;
  ...
  if (selectedPhase) {
    filteredTools = filteredTools.filter(tool =>
      tool.category?.phase === selectedPhase
    );
  }
  ...
  // Sort
  const toolsWithStatsFiltered = filteredTools;
  if (sortParam === 'name-asc') {
    ...
    filteredTools.sort((a, b) => {
      const ra = a.reviewStats?.average ?? 0;
      const rb = b.reviewStats?.average ?? 0;
      return rb - ra;
    });
  ...
  // default: newest (already sorted by getToolsWithStats)
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                {activeFilterCount > 0
                  ? `Showing ${filteredTools.length} of ${toolsWithStats.length} tools`
                  : `Browse ${toolsWithStats.length} AI tools for indie developers`}
            ...
            <div>
              ...
              <div className="space-y-2">
                <Link
                  ...
                  className={`block text-sm rounded-md px-2 py-1 -mx-2 ${!selectedPhase ? 'text-primary font-medium bg-primary/10' : 'text-muted-foreground hover:text-primary hover:bg-muted'}`}
                >
                  All Phases
                </Link>
                {PHASES.map((phase) => {
                  const isSelected = selectedPhase === phase.value;
                  const url = `/tools?phase=${phase.value}${selectedCategory ? `&category=${selectedCategory}` : ''}`;
                  return (
                    <Link
                      key={phase.value}
                      href={url as any}
